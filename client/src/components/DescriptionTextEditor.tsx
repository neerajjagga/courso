...
interface PropType<T> {
    setCourseFormData?: React.Dispatch<React.SetStateAction<T>>
    setDescription?: React.Dispatch<React.SetStateAction<string>>
    initialDescription?: string
}

const DescriptionTextEditor = <T,>({ setCourseFormData, setDescription, initialDescription = "" }: PropType<T>) => {
    ...
    const toolbarButton = (label: string, action: () => void, isActive: boolean | undefined) => (
        <button
            onClick={(e) => {
                e.preventDefault();
                action();
