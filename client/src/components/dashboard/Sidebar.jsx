...
const Sidebar = ({ isSidebarActive, setIsSidebarActive }) => {
    ...
    return (
        <div className={`md:z-50 fixed top-0 left-0 md:translate-x-0 h-screen bg-bg-primary ${isSidebarActive ? "w-[12rem] md:w-[13rem] lg:w-[16rem]" : "-translate-x-full w-[5rem] md:w-[6rem] lg:w-[6rem]"} container border-r border-gray-400 border-opacity-50 transition-all ease-in-out duration-300`}>
            <div className="flex flex-col justify-between h-screen py-6">
                <div className="flex flex-col items-center gap-20">
                    ...
                    <div className="flex flex-col gap-16">
                        {(user.role === "user" ? dashboardSidebarMenuUser : dashboardSidebarMenuInstructor).map((menu, index) => (
                            <Link to={menu.path} key={index} className="flex items-center justify-start gap-3 transition-all duration-200 ease-in-out">
                                <menu.icon />
                ...
                <div>
                    <div onClick={() => setIsProfileDropdownOpen(!isProfileDropdownOpen)} className='flex items-center justify-center gap-3 px-4 py-2 overflow-hidden text-lg transition-all duration-300 ease-in-out bg-gray-900 rounded-md cursor-pointer'>
                        <span className='py-[0.10rem] px-2 font-bold border bg-red-600 rounded-full hover:bg-red-700 hover:border-red-400 lg:text-2xl md:text-xl text-base'>{user.fullname.split('')[0].toUpperCase()}</span>
                        {isSidebarActive && (
