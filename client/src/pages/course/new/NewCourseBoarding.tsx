...
const NewCourseBoarding = () => {
    ...
    const [isBackBtnEnabled, setIsBackBtnEnabled] = useState<boolean>(false);
    const [isLiveCourseBtnEnabled, setIsLiveCourseBtnEnabled] = useState<boolean>(false);

    const [courseFormData, setCourseFormData] = useState<NewCourseFormData>({
        title: "",
        description: "",
        language: "",
        level: "",
            ...
            <div>
                ...
                <div className="fixed bottom-0 z-50 w-full py-2 pb-4 border-t border-gray-400 border-opacity-50 bg-bg-primary">
                    <div className="container flex items-center justify-between">
                        {currentStep !== 5 ? (
                            <button
                                onClick={() => currentStep !== null && navigate(`/instructor/course/create/${currentStep + 1}`)}
