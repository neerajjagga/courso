...
const CourseLandingPage = () => {
    const { courseId, titleSlug } = useOutletContext<{ courseId: string, titleSlug: string }>();
    const location = useLocation();
    const { data: course, isPending } = useFetchSingleCourse(titleSlug);

    const [courseFormData, setCourseFormData] = useState<EditCourseFormData | null>(null);
    ...
    const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        try {
            ...
            setCourseFormData((prev) => prev ? { ...prev, courseImageUrl: base64 } : null);
        } catch (error) {
    ...
    const handleSave = () => {
        if (courseFormData) {
            ...
            const updatedData: Partial<EditCourseFormData> = {};
            Object.entries(courseFormData).forEach(([key, value]) => {
                if (value !== course[key] as keyof EditCourseFormData) {
                    updatedData[key as keyof EditCourseFormData] = value;
                }
            });
        ...
        <div className='flex flex-col h-full gap-5 border border-gray-400 rounded-lg shadow-2xl md:gap-10 border-opacity-20 shadow-gray-800'>
            ...
            <div className='flex flex-col flex-wrap gap-6 px-4 pb-6 md:gap-10 sm:px-6 sm:pb-10'>
                <div className='flex flex-col w-full gap-2'>
                    ...
                    <div className='relative'>
                        <input
                            ...
                            onChange={(e) => setCourseFormData((prev) => prev ? { ...prev, title: e.target.value } : null)}
                ...
                <div className='w-full'>
                    ...
                    <DescriptionTextEditor<EditCourseFormData | null>
                ...
                <div className='flex flex-col items-center gap-4 md:items-start'>
                    ...
                    <div className='flex flex-col items-center gap-4 sm:flex-row'>
                        ...
                        {courseFormData.courseImageUrl && (
                            <button disabled={isEditingCourse} onClick={() => setCourseFormData((prev) => prev ? { ...prev, courseImageUrl: null } : null)} className='btn-danger'>Remove</button>
                        )}
                    </div>
                </div>
            </div>
